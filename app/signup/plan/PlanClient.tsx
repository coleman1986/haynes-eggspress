// app/signup/plan/PlanClient.tsx
"use client";
...
export default function PlanClient() {
  ...
  useEffect(() => {
    try {
      ...
      const savedPhone = sessionStorage.getItem("hx.phone");
      if (savedPhone) setPhone(savedPhone);
    } catch {}
  }, []);

  // Keep email/phone between visits
  useEffect(() => { try { if (email) sessionStorage.setItem("hx.email", email); } catch {} }, [email]);
  useEffect(() => { try { if (phone) sessionStorage.setItem("hx.phone", phone); } catch {} }, [phone]);
