...
export async function POST(req: NextRequest) {
  ...
  // Coverage gate
  const inArea = loadPolygonFromEnv()({
    lng: address.lng,
    lat: address.lat,
  ...
  // ⬇️ FIXED: closing braces/parens + store phone on create/update
  ...
  // Reuse customer if it already exists for this email to avoid duplicates
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    phone_number_collection: { enabled: true }, // ask Stripe to collect phone too
    // payment_method_types: ["card"], // optional; omit entirely unless you’ve enabled extras
    metadata: {
      email,
      phone: phone || "",
    },
